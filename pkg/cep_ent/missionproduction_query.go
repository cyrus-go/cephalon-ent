...
func (mpq *MissionProductionQuery) QueryHmacKeyPair() *HmacKeyPairQuery {
	query := (&HmacKeyPairClient{config: mpq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mpq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mpq *MissionProductionQuery) Clone() *MissionProductionQuery {
	...
	return &MissionProductionQuery{
		...
		withHmacKeyPair:         mpq.withHmacKeyPair.Clone(),
...
func (mpq *MissionProductionQuery) WithHmacKeyPair(opts ...func(*HmacKeyPairQuery)) *MissionProductionQuery {
	query := (&HmacKeyPairClient{config: mpq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mpq.withHmacKeyPair = query
	return mpq
}
...
func (mpq *MissionProductionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*MissionProduction, error) {
	var (
		...
		loadedTypes = [3]bool{
			...
			mpq.withMission != nil,
			mpq.withHmacKeyPair != nil,
		}
	)
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, missionproduction.ForeignKeys...)
	}
	...
	if query := mpq.withHmacKeyPair; query != nil {
		if err := mpq.loadHmacKeyPair(ctx, query, nodes, nil,
			func(n *MissionProduction, e *HmacKeyPair) { n.Edges.HmacKeyPair = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mpq *MissionProductionQuery) loadHmacKeyPair(ctx context.Context, query *HmacKeyPairQuery, nodes []*MissionProduction, init func(*MissionProduction), assign func(*MissionProduction, *HmacKeyPair)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*MissionProduction)
	for i := range nodes {
		fk := nodes[i].HmacKeyPairID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "hmac_key_pair_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mpq *MissionProductionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mpq.ctx.Fields; len(fields) > 0 {
		...
		if mpq.withHmacKeyPair != nil {
			_spec.Node.AddColumnOnce(missionproduction.FieldHmacKeyPairID)
		}
	}
