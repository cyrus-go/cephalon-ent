...
func (Bill) Fields() []ent.Field {
	return []ent.Field{
		field.Enum("type").Values("mission_consume", "mission_produce", "recharge").Default("mission_consume").StructTag(`json:"type"`).Comment("次级账单流水的类型，充值或者任务消耗或任务收益"),
		...
		field.Enum("status").Values("pending", "canceled", "done").Default("pending").StructTag(`json:"status"`).Comment("消耗流水一开始不能直接生效，确定关联的消耗时间成功后才可以扣费"),
