...
func (InvokeModelOrder) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("input_cep_cost").StructTag(`json:"input_cep_cost"`).Default(0).Comment("输入cep消耗"),
		field.Int64("output_cep_cost").StructTag(`json:"output_cep_cost"`).Default(0).Comment("输出cep消耗"),
	}
}
