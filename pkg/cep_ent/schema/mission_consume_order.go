...
func (MissionConsumeOrder) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("type").GoType(enums.MissionTypeSdTxt2Img).Default(string(enums.MissionTypeUnknown)).StructTag(`json:"type"`).Comment("任务类型，等于任务表的类型字段"),
...
func (MissionConsumeOrder) Annotations() []schema.Annotation {
	return []schema.Annotation{
		schema.Comment("任务消费订单，记录由任务消费产生的一些金额变动情况"),
	}
}
