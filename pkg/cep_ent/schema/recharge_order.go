...
func (RechargeOrder) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("campaign_order_id").Optional().Nillable().StructTag(`json:"campaign_order_id"`).Comment("活动订单 id"),
	}
}
...
func (RechargeOrder) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("campaign_order", CampaignOrder.Type).Ref("recharge_order").Field("campaign_order_id").Unique(),
	}
}
