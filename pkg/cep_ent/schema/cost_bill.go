...
func (CostBill) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("way").Values("unknow", "recharge_wechat", "recharge_alipay", "recharge_manual", "mission_timing", "mission_counting", "active_register", "active_share", "active_recharge", "first_invitee_recharge").Default("unknow").StructTag(`json:"way"`).Comment("额度账户流水的产生方式，微信、支付宝、计时消耗等"),
