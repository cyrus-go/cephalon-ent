...
func (User) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("status").GoType(enums.UserStatusNormal).Default(string(enums.UserStatusNormal)).StructTag(`json:"status"`).Comment("用户状态"),
		field.Enum("type").GoType(enums.UserTypePersonal).Default(string(enums.UserTypePersonal)).StructTag(`json:"type"`).Comment("用户类型"),
