...
func (wq *WalletQuery) QuerySymbol() *SymbolQuery {
	query := (&SymbolClient{config: wq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (wq *WalletQuery) Clone() *WalletQuery {
	...
	return &WalletQuery{
		...
		withSymbol: wq.withSymbol.Clone(),
...
func (wq *WalletQuery) WithSymbol(opts ...func(*SymbolQuery)) *WalletQuery {
	query := (&SymbolClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (wq *WalletQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Wallet, error) {
	...
	if query := wq.withSymbol; query != nil {
		if err := wq.loadSymbol(ctx, query, nodes, nil,
			func(n *Wallet, e *Symbol) { n.Edges.Symbol = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wq *WalletQuery) loadSymbol(ctx context.Context, query *SymbolQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *Symbol)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Wallet)
	for i := range nodes {
		fk := nodes[i].SymbolID
...
func (wq *WalletQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := wq.ctx.Fields; len(fields) > 0 {
		...
		if wq.withSymbol != nil {
			_spec.Node.AddColumnOnce(wallet.FieldSymbolID)
		}
	}
